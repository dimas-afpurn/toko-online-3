...
class Cilinaya_model extends CI_Model
{
	...
	function page_query($table, $order_by, $limit, $offset)
	{
		...
		if ($this->db->error()) header('Location: ../');
	...
	public function get_data_promo()
	{
		$this->db->select('*')
			->from('promo')
			->join('produk', 'promo.id_produk_promo = produk.id_produk')
			->join('kategori_produk', 'produk.kategori_produk = kategori_produk.id_kategori_produk');
		$this->db->order_by('id_produk', 'DESC');
		return $this->db->get();
	}

	public function get_data_promo_where($id_promo)
	{
		$this->db->select('*')
			->from('promo')
