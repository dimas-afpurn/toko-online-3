    ...
    <div class="_3D9BVC">
        ...
        <div class="h4QDlo" role="main" id="status_pesanan" style="display:none;">
            ...
            <div id="London" class="w3-container city" style="display:block">
                <div class="_2e9rzB">
                    <svg width="19px" height="19px" viewBox="0 0 19 19"><g id="Search-New" stroke-width="1" fill="none" fill-rule="evenodd">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#form-upload').submit(function(e) {
			...
			$.ajax({
				...
				success: function(response) {
					if(response.status) { Profil(); swal_berhasil();
					} else { swal_error(response.error); }
				},
        ...
        $('#form-alamat').submit(function(e) {
			...
			$.ajax({
				url: $(this).attr("action"), type: 'POST', dataType: 'json', data: formData, async: true,
				beforeSend: function() { $('#btnAlamat').text('saving...'); $('#BtnAlamat').attr('disabled',true); },
				success: function(response) {
					if(response.status) {  $('#modal_alamat').modal('hide'); swal_berhasil(); tampil_alamat(); 
					} else { swal_error(response.error); }
				},
				complete: function() { $('#BtnAlamat').text('save'); $('#BtnAlamat').attr('disabled',false); },
				cache: false, contentType: false, processData: false
			});
		});
        ...
        $('#form-update-alamat').submit(function(e) {
			...
			$.ajax({
				url: $(this).attr("action"), type: 'POST', dataType: 'json', data: formData, async: true,
				beforeSend: function() { $('#BtnAlamat2').text('saving...'); $('#BtnAlamat2').attr('disabled',true); },
				success: function(response) {
					if(response.status) {  $('#modal_alamat_edit').modal('hide'); swal_berhasil(); tampil_alamat(); 
					} else { swal_error(response.error); }
				},
				complete: function() { $('#BtnAlamat2').text('save'); $('#BtnAlamat2').attr('disabled',false); },
				cache: false, contentType: false, processData: false
			});
		});
		...
		$("#upload").change(function(){ readURL(this); });  
