...
class Order extends CI_Controller
{
	...
	public function simpan()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$paket_kirim 	= $this->input->post('paket_kirim');
			$total 			= $this->input->post('total_harga');
			$jumlah 		= $this->input->post('total_jumlah');
			$ongkir			= $this->input->post('ongkir');
			$lama_kirim 	= $this->input->post('lama_kirim');
			$email			= $this->input->post('email');
			$tgl_order 		= date("Y-m-d H:i:s");


			$batas_bayar = $this->toko_online_model->get_table('waktu_bayar');
			$batas = "+" . $batas_bayar[0]['waktu'] . " " . $batas_bayar[0]['mode'];
			$expired = date('Y-m-d H:i:s', strtotime($batas . $tgl_order));
			...
			if ($this->session->userdata('id_member') == "") {
				$data_insert = array(
					...
					'total_order'	=> $total,
					'jumlah_order'  => $jumlah,
					'subtotal_order'	=> $this->input->post('total_harga_sub'),
					'status_order'	=> 1,
					'nama_order'	=> $nama,
					'tlp_order'		=> $phone,
					'provinsi'      => $provinsi,
			...
			if ($insert_order) {
				...
				$data_order = $this->toko_online_model->get_keranjang_belanja(array('keranjang_belanja.id_keranjang_belanja' => $this->session->userdata('id_kpesan')));
				foreach ($data_order as $d) {
					...
					if ($cek_data == null) {
						...
						$data = array(
							...
							'berat_produk'	=> $d['jumlah_produk'] * $d['berat_kotor'],
						...
						if ($insert_data) {
							...
							$where = array('id_keranjang_belanja' => $this->session->userdata('id_kpesan'));
				...
				$this->session->unset_userdata('id_kpesan');
				redirect(base_url('payment/thank-you/' . $next_id_order));
			} else {
	...
	public function konfirmasi_pembayaran()
	{
		...
		$data['title'] = 'Konfirmasi Pembayaran | blonjosam.com';
		$data['nav'] = 4;
		// $this->load->view('user/dashboard1', $data);
		echo json_encode($data);
	}
	...
	public function search_order()
	{
		...
		$transaksi = $this->toko_online_model->get_table_where('transaksi_midtrans', array('id_order' => $id_order));
		$batas_bayar = $this->toko_online_model->get_table('waktu_bayar');
		if ($data_order != null || $penjual != null) {
			...
			$batas = "+" . $batas_bayar[0]['waktu'] . " " . $batas_bayar[0]['mode'];
			$expired = strtotime($batas, $tgl_order);
			if (time() - $timestamp > $expired) {
				if (($data_order[0]['status_bayar']) == 1) {
					...
					if ($data_order[0]['status_order'] == 1) {
						$data['status_order'] = "Menunggu pembayaran";
	...
	{
		$data['title'] = "Pembayaran | blonjosam.com";
		...
		$data['expired'] = $this->toko_online_model->get_table('waktu_bayar');
		$this->load->view('user/dashboard1', $data);
	}
	...
	public function refund_uang()
	{
		$id_detail_order = $this->input->post('id_detail_order');
		$data_detail_order = $this->toko_online_model->get_order(array('id_detail_order' => $id_detail_order));
		$data_order = $this->toko_online_model->get_table_where2('order', array('id_order' => $data_detail_order[0]['id_order']));
		$data_penjual = $this->toko_online_model->get_table_where2('user', array('id_user' => $data_detail_order[0]['id_penjual']));
